...
class LLMNode(BasePlatformNode, Node):
    ...
    PARAMS = {
        "api_base": {
            "type": "string",
            "description": "API base URL (e.g., http://localhost:1234/v1)",
        },
        "api_key": {
            "type": "string",
            "description": "API key (usually 'lm-studio' or similar)",
        },
        "model": {"type": "string", "description": "Model name (e.g., 'llama-3.2')"},
        "system_prompt": {"type": "string", "description": "System prompt for the LLM"},
        "user_prompt": {
            "type": "string",
            "description": "User prompt (can use {input}, {memory_key})",
        },
        "temperature": {
            "type": "float",
    ...
    def prep(self, shared):
        ...
        return {"context": context, "history": history, "shared": shared}
    ...
    def exec(self, prep_res):
        ...
        try:
            client = openai.OpenAI(
                base_url=self.api_base, api_key=self.api_key, timeout=600
            )
            ...
            print(
                f"Sending request to {self.api_base} with model {self.model} ({len(messages)} messages)"
            )
            ...
            return {"response": content, "user_message": user_content, "success": True}
    ...
    def post(self, shared, prep_res, exec_res):
        ...
        if self.use_history and exec_res.get("success"):
            ...
            self._save_history(
                self.conversation_id, user_msg, response, self.max_history
            )
    ...
    def _save_history(
        self, conversation_id: str, user_msg: str, assistant_msg: str, max_history: int
    ):
